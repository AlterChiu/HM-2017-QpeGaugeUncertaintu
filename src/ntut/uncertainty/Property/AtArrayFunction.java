...
public class AtArrayFunction {
	...
	public String[][] getWithoutColumn(int order) {
		String[][] temp = new String[this.content.length][content[0].length - 1];
		for (int i = 0; i < temp.length; i++) {
			for (int j = 0; j < temp[i].length; j++) {
				if (j != order && order < j) {
					temp[i][j - 1] = this.content[i][j];
				}else if(j!=order && order>j){
	...
	public String[][] getWithoutColumn(int order1 ,int order2) {
		String[][] temp = new AtArrayFunction(this.content).getWithoutColumn(order1); 
		temp = new AtArrayFunction(temp).getWithoutColumn(order2); 
		return temp;
	}
	 ...
	 * 
	 * @param order
	 *            : base on which column
	 * @param spaceNum
	 *            : how many space between max and min value
